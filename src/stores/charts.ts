...
export const useChartsStore = defineStore("charts", () => {
  ...
  async function createChart(
    ...
    recordHistory("create", `创建图表: ${title}`);
  ...
  async function updateChart(chart: ChartConfig) {
    ...
    if (index !== -1) {
      ...
      recordHistory("update", `更新图表: ${chart.title}`);
    }
  }

  async function setSlot(
    slotName: keyof ChartSlots,
    ...
    if (slotName === "y") {
      if (
        field &&
        !updatedSlots.y.find((y) => y.fieldName === field.fieldName)
      ) {
  ...
  async function removeFromSlot(
    ...
    if (slotName === "y" && fieldName) {
      ...
      (updatedSlots as any)[slotName] =
        slotName === "y" ? [] : slotName === "filters" ? [] : null;
    }
  ...
  async function removeFilter(index: number) {
    ...
    const newFilters = activeChart.value.slots.filters.filter(
      (_, i) => i !== index,
    );
  ...
  async function deleteChart(id: string) {
    ...
    if (index !== -1) {
      ...
      recordHistory("delete", `删除图表: ${chartTitle}`);
    }
  }
  ...
  async function renameChart(id: string, newTitle: string) {
    ...
    if (chart) {
      ...
      await idb.saveChart(chart);
      recordHistory("rename", `重命名图表: ${newTitle}`);
    }
  }
