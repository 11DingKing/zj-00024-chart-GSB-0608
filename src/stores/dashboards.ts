...
export const useDashboardsStore = defineStore('dashboards', () => {
  ...
  const activeTab = computed(() => {
    ...
    return activeDashboard.value.tabs[activeDashboard.value.activeTabIndex] || null
  })
  ...
  async function createDashboard(name: string = 'New Dashboard'): Promise<Dashboard> {
    const now = Date.now()
  ...
  async function removeTab(tabId: string) {
    ...
    const tabIndex = activeDashboard.value.tabs.findIndex((t) => t.id === tabId)
  ...
  async function addChartToDashboard(chartId: string) {
    ...
    const maxX = existingLayout.reduce((max, item) => Math.max(max, item.x + item.w), 0)
  ...
  async function removeFromLayout(itemId: string) {
    ...
    const newLayout = activeTab.value.layout.filter((item) => item.i !== itemId)
    await updateLayout(newLayout)
  }
